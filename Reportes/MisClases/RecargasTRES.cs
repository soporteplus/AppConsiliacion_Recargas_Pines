    ...
    class RecargasTRES
    {
        ...
        public void ConciliarRecargas(ProgressBar PBar, string ColumnasManejador, DataGridView DGVManager, DataGridView DGVTicketsFound, DataGridView DGVTicketsNotFound, DataGridView DGVEmpates, DataGridView DGVMatchRecargas, DataGridView DGVNoMachtRecargas, DataGridView DGVTempRECARGAS, DataGridView DGVCajas, DataGridView DGVRecargas, DataGridView DGVTickets, DataGridView DGVReporte, DataGridView DGVRecargasMTC)
        {
            ...
            if (Carpeta.ShowDialog() == DialogResult.OK)
            {
                ...
                DatosMNGRRecargas(DireccionCarpeta + "\\00.xls", ColumnasManejador, DGVManager, DGVTicketsFound, DGVTicketsNotFound, DGVEmpates, DGVRecargas);
        ...
        public void DatosMNGRRecargas(string ExcelName, string ColumnasManejador, DataGridView DGVManager, DataGridView DGVTicketsFound, DataGridView DGVTicketsNotFound, DataGridView DGVEmpates, DataGridView DGVRecargas)
        {
            //try
            //{
            ...
            if (tICKETS == true)
            {
                for (int fila = 0; fila < DGVManager.Rows.Count; fila++)
                    ...
                    for (int FilaT = 0; FilaT < NoTickets; FilaT++)
                    {
                        ...
                        {
                            //     string Texto = DGVManager.Rows[fila].Cells[0].Value.ToString();
            ...
            for (int FUnidades = 1; FUnidades < DGVManager.Rows.Count; FUnidades++)
                ...
                int val = Convert.ToInt32(DGVManager.Rows[FUnidades].Cells[10].Value.ToString());
                if (val < 0)
                {
                    val = val * -1;
                }
            ...
            RecargasMNGR = new string[NoRecargas+1, 5];
                ...
                if (unidades != 0 && DGVManager.Rows[fila].Cells[10].Value.ToString() != "")
                {
                    ...
                    if (unidades > 1)
                    {
                        for (int unit = 0; unit < unidades; unit++)
                        {
                            ...
                            RecargasMNGR[NDato, 3] = Concepto(DGVManager.Rows[fila].Cells[0].Value.ToString(), DGVRecargas);//Recarga
        ...
        public void Recargas_MTC(DataGridView DGVRecargasMTC, DataGridView GVRecargasMTC, DataGridView DGVCajas, DataGridView DGVRecargas)
        {
            ...
            for (int i = 0; i < NRecargasMTC; i++)
            {
                ...
                for (int filaCajas = 1; filaCajas < DGVCajas.Rows.Count; filaCajas++)
                {
                    ...
                    if (CajaCatalogo.Contains(tienda))
                    //if (tienda == CajaCatalogo.Contains(tienda))
                    {
                ...
                for (int CCSRC = 0; CCSRC < DGVRecargas.Rows.Count/*NoSer*/; CCSRC++)
        ...
        public void NoMatchR(DataGridView DGVNoMachtRecargas)
        {
            ...
            for (int y = 0; y < RecargasMTC.GetLength(0) ; y++)
            {
                cajaMTC = RecargasMTC[y, 0].ToString();
                DateTime FMTC = Convert.ToDateTime(RecargasMTC[y, 1].ToString().Replace("p.m.", "PM").Replace("a.m.", "AM"));
            ...
            {
                string cajaMngr = RecargasMNGR[x, 0].ToString();
                if (cajaMTC == cajaMngr && RecargasMNGR[x, 4] != "MATCH")
                {
                    DateTime FMNGR = Convert.ToDateTime(RecargasMNGR[x, 1].ToString().Replace("p.m.", "PM").Replace("a.m.", "AM"));
                    //Información Manager
                    DGVNoMachtRecargas.Rows.Add("Manager", RecargasMNGR[x, 0].ToString(), FMNGR.ToString(), RecargasMNGR[x, 2].ToString(), RecargasMNGR[x, 3].ToString());
